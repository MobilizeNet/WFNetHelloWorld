namespace WebSite
{
    ...
    {
        private static List<Assembly> assembliesForRegistrations = Mobilize.WebMap.Common.Core.AssemblyRegistration.GetAssembliesForRegistration(Assembly.GetEntryAssembly());

        /// <summary>
        /// Registers the model projectors.
        /// </summary>
        /// <param name="services">The services.</param>
        public static void RegisterModelProjectors(this IServiceCollection services)
        {
            services.AddSingleton<IMapperCatalog>((provider) =>
            {
                ...
                Mobilize.WebMap.Common.Core.AssemblyRegistration.RegisterMappers(assembliesForRegistrations, logger.CreateLogger("Information"), service);
        ...
        public static void RegisterWrappers(this IServiceCollection services)
        {
            services.AddSingleton<IObservableWrapperCatalog>(
              (provider) =>
              {
                  ...
                  Mobilize.WebMap.Common.Core.AssemblyRegistration.RegisterWrappers(assembliesForRegistrations, catalog);
